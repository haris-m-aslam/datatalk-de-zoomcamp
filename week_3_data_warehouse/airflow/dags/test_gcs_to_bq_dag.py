...
with DAG(
    ...
    for colour, ds_col in COLOUR_RANGE.items():
        move_files_gcs_task = GCSToGCSOperator(
            ...
            delimiter="{INPUT_FILETYPE}",
            destination_bucket=BUCKET,
            destination_object=f'test_2/{colour}_{DATASET}',
            move_object=False
        )
        ...
        bigquery_external_table_task = BigQueryCreateExternalTableOperator(
            ...
            table_resource={
                "tableReference": {
                    ...
                    "tableId": f"{colour}_{DATASET}_external_table_test",
                },
                "externalDataConfiguration": {
                    "autodetect": "True",
                    "sourceFormat": "PARQUET",
                    "sourceUris": [f"gs://{BUCKET}/test_2/{colour}_{DATASET}*"],
                },
            },
        )

        CREATE_BQ_TBL_QUERY = (
            f"CREATE OR REPLACE TABLE {BIGQUERY_DATASET}.{colour}_{DATASET}_partitioned_test \
            PARTITION BY DATE({ds_col}) \
            AS \
            SELECT * FROM {BIGQUERY_DATASET}.{colour}_{DATASET}_external_table_test;"
        )
